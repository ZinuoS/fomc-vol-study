"""
Convert fomc_straddle_notebook.py → fomc_straddle_notebook.ipynb
"""
import re, json
from pathlib import Path
from uuid import uuid4

SRC  = Path("fomc_straddle_notebook.py")
DEST = Path("fomc_straddle_notebook.ipynb")
...
for kind, label, body in cells_raw:
    ...
    if kind == "markdown":
        ...
        code = "".join(body)
        # Straddle notebook uses ipywidgets — keep matplotlib inline, don't swap backend
        source = code.rstrip("\n")
        if source.strip():
